...
class PacketHandler : public Napi::ObjectWrap<PacketHandler> {
 public:
  ...
  PacketHandler(const Napi::CallbackInfo& info)
    : Napi::ObjectWrap<PacketHandler>(info),
      transport({
          .write = [this](gsl::span<const gsl::byte> bytes)
          {
              write.Call({ Napi::ArrayBuffer::New(env, reinterpret_cast<void*>(const_cast<gsl::byte*>(bytes.data())), bytes.size()) });
              return true; // TODO: write.Call return type
          },
          .onPacketReceived = [this](gsl::span<const gsl::byte> bytes) { onPacketReceived(bytes); }
      }),
      codec(transport, 20), // TODO: Need to know MTU
      env(info.Env()),
      emit(Napi::Persistent(info[0].As<Napi::Function>())),
      write(Napi::Persistent(info[1].As<Napi::Function>()))
   {
   }
 ...
 private:
    struct Transport {
        using WriteFunction = std::function<bool(gsl::span<const gsl::byte>)>;
        using PacketCallbackType = std::function<void(gsl::span<const gsl::byte>)>;

        struct Callbacks {
            WriteFunction write;
            PacketCallbackType onPacketReceived;
        };

        Transport(const Callbacks& cbs) : callbacks(cbs) { }

        bool transmitPacket(gsl::span<const gsl::byte> bytes)
        {
            return callbacks.write && callbacks.write(bytes);
        }
        ...
        void receiveMessage(const Packet &packet) const
        {
            if (callbacks.onPacketReceived)
                callbacks.onPacketReceived(gsl::as_bytes(gsl::make_span(packet)));
        }
  ...
  void SendQuery(const Napi::CallbackInfo& info);

  //====================================================================================================================
  void onPacketReceived(gsl::span<const gsl::byte> bytes)
  {
